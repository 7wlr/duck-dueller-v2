...
object LobbyMovement {
        ...
        if (wlr.mc.thePlayer != null) {
            ...
            TimeUtils.setTimeout(fun () {
                ...
                intervals.add(TimeUtils.setInterval(fun () {
                    tickYawChange = if (WorldUtils.airInFront(wlr.mc.thePlayer, 7f)) {
                        if (WorldUtils.airInFront(wlr.mc.thePlayer, 3f)) {
    ...
    fun onTick(ev: ClientTickEvent) {
        if (wlr.bot?.toggled() == true && tickYawChange != 0f && wlr.mc.thePlayer != null && StateManager.state != StateManager.States.PLAYING) {
